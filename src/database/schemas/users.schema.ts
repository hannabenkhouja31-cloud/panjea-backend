...
export const users = pgTable('users', {
  ...
  pets: text('pets').array().default(sql`'{}'::text[]`),
  habits: text('habits').array().default(sql`'{}'::text[]`),
  travelMemory: text('travel_memory'),
  travelTriggers: text('travel_triggers').array().default(sql`'{}'::text[]`),
  travelBlockers: text('travel_blockers'),
  travelerEnergy: text('traveler_energy').array().default(sql`'{}'::text[]`),
  travelSuperpowers: text('travel_superpowers').array().default(sql`'{}'::text[]`),
  travelSuperpowerText: text('travel_superpower_text'),
  travelVision: text('travel_vision').array().default(sql`'{}'::text[]`),
  travelValues: varchar('travel_values', { length: 100 }),
  minAgePreference: smallint('min_age_preference'),
  maxAgePreference: smallint('max_age_preference'),
  profileCompleted: boolean('profile_completed').notNull().default(false),
  lastCompletedStep: smallint('last_completed_step').notNull().default(2),
}, (table) => ({
  ...
  agePreferenceCheck: check('users_age_preference_check', sql`${table.minAgePreference} IS NULL OR ${table.maxAgePreference} IS NULL OR ${table.maxAgePreference} >= ${table.minAgePreference}`),
}));